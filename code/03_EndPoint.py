        ...
        try:
            ...
            for run in runs:
                run_info = {
                    ...
                    "sarima_order": f"({run.data.params.get('p', '?')},{run.data.params.get('d', '?')},{run.data.params.get('q', '?')})"
    ...
    def auto_load_best_models(self):
        ...
        for segment in VALID_SEGMENTS:
            for target in VALID_TARGETS:
                matching_runs = [
                    r for r in self.available_runs 
                    if r["segment"] == segment and r["target"] == target
                ]
...
def list_loaded_models():
    return jsonify({
        ...
        "count": len(model_manager.current_models)
    })
...
def load_model():
    ...
    if segment not in VALID_SEGMENTS:
        return jsonify({
            "error": f"Invalid segment. Must be one of: {VALID_SEGMENTS}"
        }), 400
...
def predict():
    ...
    if not isinstance(num_periods, int) or num_periods < 1 or num_periods > 24:
        return jsonify({
            "error": "num_periods must be an integer between 1 and 24"
        }), 400
...
def predict_batch():
    ...
    for i, date in enumerate(forecast_dates):
        ...
        for target, preds in results.items():
            entry[f"{target}_forecast"] = round(preds[i], 2)
        
        if "net_sales" in results and "returns" in results:
            entry["total_net_forecast"] = round(max(results["net_sales"][i] - results["returns"][i], 0), 2)
